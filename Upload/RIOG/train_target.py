...
def mean_dicts(dict_1):
    dict_1_values = [dict_1[key] for key in sorted(dict_1.keys())]
    dict_1_tensor = torch.cat(tuple([t.view(-1) for t in dict_1_values]))
    dict_1_tensor = dict_1_tensor.abs().mean()
    return (dict_1_tensor).item()

import gc
def get_modelgrad(model_t, model_s, model_gb, train_loader, args):
    ...
    for sample_w, sample_s in train_loader:
        ...
        loss_seg_pixel = bce_loss(predictions_stu_s_sigmoid, pseudo_labels)

        # loss_seg = torch.mean(loss_seg_pixel)

        loss_ent = entropy_loss(predictions_stu_s_sigmoid).mean()

        pw = model_gb(predictions_stu_s_sigmoid)

        loss_gb_pixel = F.mse_loss(pw, predictions_tea_w_sigmoid, reduction='none')
        loss0 = torch.mean(loss_seg_pixel[:,0,:,:] + 0.2*loss_gb_pixel[:,0,:,:])
        loss1 = torch.mean(loss_seg_pixel[:,1,:,:] + 0.2*loss_gb_pixel[:,1,:,:])

        r0 = get_grads(loss0, optim, model_s.decoder)
        r1 = get_grads(loss1, optim, model_s.decoder)

        weight = r0/r1
        weightes.append(weight)


    return sum(weightes)/len(weightes)
...
def adapt_epoch(model_t, model_s, optim, model_gb, train_loader, args, weight):
    for sample_w, sample_s in train_loader:
        imgs_w = sample_w['image']
        imgs_s = sample_s['image']
        img_name = sample_w['img_name']
        ...
        loss_seg_pixel = bce_loss(predictions_stu_s_sigmoid, pseudo_labels)# + bce_loss(pseudo_labels, predictions_stu_s_sigmoid)

        pw = model_gb(predictions_stu_s_sigmoid)

        loss_gb_pixel = F.mse_loss(pw, predictions_tea_w_sigmoid, reduction='none')

        # loss = torch.mean(loss_seg_pixel)# + 0.1*torch.mean(loss_gb_pixel)
        # loss.backward()
        # optim.step()
        # optim.zero_grad()
        ...
        loss0 = torch.mean(loss_seg_pixel[:,0,:,:] + 0.2*loss_gb_pixel[:,0,:,:])
        loss1 = torch.mean(loss_seg_pixel[:,1,:,:] + 0.2*loss_gb_pixel[:,1,:,:])
        losses = [loss1, (1/weight)*loss0]
        optim.pc_backward(losses)
        optim.step()
        optim.zero_grad()
...
def main():
    ...
    checkpoint2 = torch.load("/model_gb_checkpoint_1.pth.tar")
    model_gb.load_state_dict(checkpoint2['model_state_dict'])
    ...
    for epoch in range(args.epoch):
        ...
        r = get_modelgrad(model_t, model_s, model_gb, train_loader, args)
        ratio_save[epoch] = 1/r
        # print(r)
        # r = 1

        # if epoch == 0:
        #     configure_model(model_gb)
        # else:
        #     model_gb.eval()
        #     model_gb.requires_grad_(False)


        adapt_epoch(model_t, model_s, optim, model_gb, train_loader, args, r)
